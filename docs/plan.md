...
### Milestone 4: Automated Data Ingestion & Account Discovery [COMPLETED ✅]
* **Unit M4.1**: Implemented `src/fetcher.rs` with `reqwest` + `scraper` (gated behind `web-scraper` Cargo feature) to fetch AAII Sentiment Survey and NAAIM Exposure Index. Returns `None` on scrape failure (no false signalling).
